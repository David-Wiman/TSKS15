function [new_melody_id_guess, pitch] = SingleToneClassifier(signal)

    y = signal; % vector with 12 notes, 43392 samples total
    K = 3616; % number of samples per note
    NR_OF_TONES = 2; % each tone split into cos and sin
    ...
    for n = 1:12
        for j = 1:20
            ...
            % sin
            H(j,n,:,:) = [cos(2*pi*(0:K-1)'*f(j,n)) sin(2*pi*(0:K-1)'*f(j,n))];
        end
    end
